...
def createSchemesForVariants():
    ...
    for variant in variants:
        ...
        globalSchemeFile = variantsFolder.go([variant, "theme", "active", "scheme", "global.ini"])
        result = Mixer(
...
def createSchemesForResolutions():
    ...
    for resolution in supportedResolutions:
        ...
        defaultSchemeFile = auroraFolder.go([resolution, "scheme"]).makeDir().go("default.ini")
        result = Mixer(
            template=defaultSchemeTemplate.read(),
            data=resolutionsDataFile.read()
        ).mix()
        defaultSchemeFile.write(result)
        Console.success(f"Schemes created for {resolution}.")
...
def zipVariants():
    ...
    for variant in variants:
        ...
        Console.success(f"{variant} variant zipped.")
