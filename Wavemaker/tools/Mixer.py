...
class Mixer:
	...
	def __interpretToml(tomlString: str) -> dict[str, str | bool]:
		try:
			...
			logger.critical(f"Invalid TOML : {error}")
			exit(os.EX_DATAERR)

	@staticmethod
	def __removeCommentaries(template: str) -> str:
		return re.sub("//.*?//", lambda _: "", template)
	...
	def __replaceConditionals(template: str, data: dict[str, bool]) -> str:
		return re.sub(
			"\\[\\[(.+?)\\?(.+?):(.+?)]]",
			lambda m: str(
				m.group(2) if bool(data.get(m.group(1), False)) else m.group(3)
			),
			template,
		)
	...
	def __removeInvalidConditionals(template: str) -> str:
		return re.sub("\\[\\[.*?]]", lambda m: "##INVALID##", template)
